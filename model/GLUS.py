...
class GlusMetaModel:
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.config = config
        self.config.mask_threshold = 0.0 # 掩码生成阈值
        self.config.max_hole_area = 0.0  # 最大孔洞面积限制
        self.config.max_sprinkle_area = 0.0 # 最大散点面积限制
        self.sam_config = kwargs.get("sam_config", None) # SAM模型配置路径
    ...
    def initialize_glus_modules(self, config):
        # SAM
        # 包含SAM视觉模型和文本投影层
        self.visual_model = build_sam2(config_file=self.sam_config, ckpt_path=self.vision_pretrained) # SAM图像分割模型
        ...
        # 文本到视觉空间的投影网络
        text_fc = [
            nn.Linear(in_dim, in_dim), # 维度保持
            nn.ReLU(inplace=True), # 非线性激活
            nn.Linear(in_dim, out_dim), # 降维到视觉空间
            nn.Dropout(0.0),
        ]
...
""" 
      ...
      主模型类，继承自GlusMetaModel和LlavaLlamaModel
"""    
...
class GLUSForCausalLM(LlavaLlamaForCausalLM):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.ce_loss_weight = kwargs.pop("ce_loss_weight", None) #交叉熵损失
        self.dice_loss_weight = kwargs.pop("dice_loss_weight", None) #dice分割损失
        ...
        if not hasattr(config, "train_mask_decoder"):
            ...
            config.mm_vision_tower = kwargs.get(
                "vision_tower", "/2025900123/yrr/GLUS-main/checkpoints/clip-vit-large-patch14"
            )
        ...
        super().__init__(config) # 初始化父类(LlavaLlamaForCausalLM)

        self.model = GlusModel(config, **kwargs) # 创建GLUS模型实例
    ...
    def get_visual_embs(self, pixel_values: torch.FloatTensor, frame_id: int, bt_sz: List[int], mem_stride = 1):
        ...
        with torch.no_grad():
            ...
            for i in range(pixel_values.shape[0]):
                ...
                _, vision_feats_per, vision_pos_embeds_per, _ = self.model.visual_model._prepare_backbone_features(backbone_out) #  返回 [低分辨率特征, 高分辨率特征]
